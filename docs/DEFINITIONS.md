...
`{dir: "in"|"out", kind?, text}` — §8), `store` (memory store profile,
tier placement, record write, or commit:
`{op?, name?, available?, latencyMs?, capabilities?, placement?, tier?, ref?, sha?, reason?}`
— §7).
